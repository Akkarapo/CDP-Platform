...
function decodeGoogleUser(token: string): AuthUser | null {
  ...
  const issuerIsGoogle = payload?.iss === "https://accounts.google.com" || payload?.iss === "accounts.google.com";
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    function init() {
      ...
      window.google.accounts.id.initialize({
        ...
        callback: (response: { credential?: string }) => {
          const googleUser = response.credential ? decodeGoogleUser(response.credential) : null;
          if (!googleUser) {
            setError("Google ไม่สามารถยืนยันบัญชีนี้ได้ กรุณาลองเข้าสู่ระบบอีกครั้ง");
            return;
          }
          setError(null);
          setUser(googleUser);
          localStorage.setItem(LS_KEY, JSON.stringify(googleUser));
        },
        auto_select: false,
    ...
    script.onload = init;
    script.onerror = () => {
      setError("โหลด Google Sign-In ไม่สำเร็จ กรุณาตรวจสอบอินเทอร์เน็ตแล้วรีเฟรชหน้า");
      setReady(true);
    };
  ...
  useEffect(() => {
    if (!user || user.sub === "dev") return;
    const delay = Math.max(0, user.expiresAt - Date.now());
  ...
  return (
    <AuthContext.Provider value={{ user, ready, error, clientIdConfigured: Boolean(CLIENT_ID), signOut, devSignIn }}>
      {children}
...
export function GoogleSignInButton() {
  ...
  return <div ref={divRef} aria-label="เข้าสู่ระบบด้วย Google" />;
}
